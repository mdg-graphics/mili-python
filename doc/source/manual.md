...
The distinction between Serial and Parallel databases is important because the Mili Python Reader behaves differently depending on whether you are reading a Serial or Parallel database. These differences are described in detail throughout the manual.
...
- [Parallel vs. Serial Databases](#parallel-vs-serial-databases)
...
This function will return a `MiliDatabase` object or a parallel wrapper. The `MiliDatabase` class provides an api for interacting with Mili databases.
...
```
**Description**: Get list of classes of a specified material name/number.

**Arguments**:
    - `mat`: The material name or number.

**Returns**:
    - `List[str]`: A list of class names associated with the material.
...
```
**Description**: Get the available integration points for each material.

**Arguments**:
    - `None`

**Returns**:
    - `Dict`: Dictionary mapping material numbers to integration points.
...
> **WARNING**: The functions below are available for users, but it is unlikely that user's should need to use them. These functions are mostly getters for the internal dictionaries and lists of the `MiliDatabase` object and are used by the functions above to get information out of the database.
...
```
**Description**: Getter for internal materials dictionary. Maps material names to material numbers.

**Arguments**:
    - `None`

**Returns**:
    - `Dict`: The materials dictionary.

## state_variables
```python
def state_variables(self) -> List[StateVariable]
```
**Description**: Getter for internal list of StateVariable objects.

**Arguments**:
    - `None`

**Returns**:
    - `List[StateVariable]`: List of state variable objects.

## subrecords
```python
def subrecords(self) -> List[Subrecord]
```
**Description**: Getter for internal list of Subrecord objects.

**Arguments**:
    - `None`

**Returns**:
    - `List[Subrecord]`: List of subrecord objects.
...
```
**Description**: Getter for internal integration points dictionary.

## element_sets
```python
def element_sets(self) -> Dict
```
**Description**: Getter for element sets.
