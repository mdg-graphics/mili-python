...
import platform
from mili_reader_lib import Mili, DirectoryType, Superclass, ConnWords, DataType, ExtSize, AggregateType, DataOrganization
...
class MiliCombiner:
    def __init__(self, base_name, parameters, files=None):
        
        if files == None or len(files)<2:
        ...
        self.directories = mili.getDirectories()
        print (self.directories.keys())
        for key in self.directories.keys():
            print("proc: ",key)
            print(self.directories[key])
        
        self.master_index = [0]*4
        
        print (type(self.timestep_maps))
    ...
    def __CommitDirectories(a_file, offset, header_array, global_dirs, dir_order):    
    ...
    def __CommitMesh(a_file, offset, header_array, global_dirs, global_dir_strings, mesh_dim, floats, 
                     elem_conns, node_local_id_to_global_id, global_class_idents):
    ...
    def __CommitSvars(a_file, offset, global_dirs, global_svar_s, global_svar_ints):
    ...
    def __CommitNames(a_file, offset, global_dir_strings, global_dirs):
        ...
        mult = __MakeMultOfFour(all_string_sum)
        all_dir_strings += "\x00"*(mult-len(all_dir_strings))
        all_string_sum += mult-len(all_dir_strings)
        all_dir_strings = bytes(all_dir_strings, 'ascii')
        # Round all_string_sum up to a multiple of four if not
        
        a_file.seek(offset)
        #print(offset, a_file.tell())
    ...
    def __CommitParams(a_file, offset, global_params, global_dirs, global_dir_strings, 
                       global_node_labels, global_elem_labels, global_elem_offsets,     
                       header_array, dim, floats, elem_conns, node_local_id_to_global_id, global_class_idents):
    ...
    def __CommitSubrecords(a_file, offset, global_dirs, global_dir_strings, global_svar_s, 
                           global_svar_ints, global_svar_inners, global_subrec_idata, 
                           global_subrec_cdata):
    ...
    def __ReadMesh(p, f, this_proc_dim, this_indexing_array, this_proc_dirs, 
                   dir_strings_dict, global_class_idents, global_dir_dict, 
                   global_dir_string_dict, node_label_consec_global_ids, this_proc_connectivity, 
                   this_proc_materials, this_proc_labels, global_floats_consec, have_label, 
                   add_node, node_local_id_to_global_id, uniq_nodes):
    ...
    def __ReadNames(p, f, file_tag, offset, this_indexing_array, this_proc_dirs, 
                    dir_num_strings_dict, global_names):
        ...
        for dir_type in this_proc_dirs.keys():
    ...
    def __ReadIndexing(file,this_header_array):
        ...
        byte_array = file.read(16)
        
        return struct.unpack(query_string, byte_array)
    ...
    def __StartRead(p, file_name, header_array, indexing_dict, dims, global_state_maps, 
                    global_class_idents, global_names, global_params, global_svar_header, 
                    global_svar_s, global_svar_ints, global_svar_inners, global_dir_dict, 
                    global_dir_string_dict, global_elem_conns_dict, global_labels, have_label, 
                    add_node, global_subrec_idata, global_subrec_cdata, global_srec_headers, 
                    node_label_consec_global_ids, node_local_id_to_global_id, 
                    elem_processor_offsets, global_floats_consec, uniq_nodes):
        ...
        with open(file_name, 'rb') as f:
            ...
            index_info = __ReadIndexing(f,local_header)
            ...
            if p == 0:
                ...
                index_array = __InitWrite(afile_name, mili_taur, local_header)
                print(index_info)
                offset = __ReadStateMaps(f, global_state_maps)
            
            offset = -16 # can assume P0 will always set offset, so can leave as uninitialized var above...
            offset = __ReadDirectories(p, f, file_size, offset, local_file_tag, local_header, indexing_dict, 
                                       local_dir_strings_dict, local_directory_dict)
    
            if local_indexing[IndexingArray.NULL_TERMED_NAMES_BYTES.value] > 0:
                __ReadNames(p, f, local_file_tag, offset, local_indexing, local_directory_dict, 
                            local_dir_strings_dict, global_names)
                this_proc_dim = __ReadParams(p, f, local_file_tag, local_directory_dict, local_dir_strings_dict, 
                                             global_params, this_proc_dim, this_proc_labels, this_proc_matname, 
                                             this_proc_i_points, this_proc_param_dirs, global_dir_dict, 
                                             global_dir_string_dict)
                dims[p] = this_proc_dim
                
                __ReadSvars(p, f, local_directory_dict, global_svar_header, global_svar_s, global_svar_ints, 
                            global_svar_inners)
                
                __ReadMesh(p, f, this_proc_dim, local_indexing, local_directory_dict, local_dir_strings_dict, 
                           global_class_idents, global_dir_dict, global_dir_string_dict, node_label_consec_global_ids, 
                           local_connectivity, local_materials, this_proc_labels, global_floats_consec, have_label, 
                           add_node, node_local_id_to_global_id, uniq_nodes)
    
                __RemapConns(p, f, local_directory_dict, local_dir_strings_dict, this_proc_labels, 
                             node_label_consec_global_ids, global_elem_conns_dict, global_labels, have_label, 
                             elem_processor_offsets, uniq_nodes)
                
                __ReadSubrecords(p, f, local_directory_dict, local_dir_strings_dict, node_label_consec_global_ids, 
                                 global_elem_conns_dict, this_proc_labels, global_labels, global_srec_headers, 
...
def main():
    ...
    #print(file_names)
    combiner = Combiner(params["input_file"],params,files=file_names)
    #print(dir(combiner))
    sys.exit(0)
    manager = Manager()
    ...
    global_names = manager.dict() # not necessary
...
#################### Commits ######################
    ...
    with open(afile_name, 'ab+') as a_file:        
        ...
        curr_wrt_index = __CommitParams(a_file, curr_wrt_index, global_params, 
                                        global_dir_dict, global_dir_string_dict, 
                                        node_labels, global_labels, elem_processor_offsets,    
                                        header_array, dims[0], floats, global_elem_conns_dict, 
                                        node_local_id_to_global_id, global_class_idents)
        
        
        # Called in CommitParams
        curr_wrt_index = __CommitMesh(a_file, curr_wrt_index, header_array, global_dir_dict, 
                                      global_dir_string_dict, dims[0], floats, global_elem_conns_dict, 
                                      node_local_id_to_global_id, global_class_idents)

        curr_wrt_index = __CommitSvars(a_file, curr_wrt_index, global_dir_dict, global_svar_s, global_svar_ints)
        
        curr_wrt_index = __CommitSubrecords(a_file, curr_wrt_index, global_dir_dict, 
                                            global_dir_string_dict, global_svar_s, 
                                            global_svar_ints, global_svar_inners, 
                                            global_subrec_idata, global_subrec_cdata)

        curr_wrt_index, null_termed_names_bytes, dir_order =  __CommitNames(a_file, curr_wrt_index, 
                                                                            global_dir_string_dict, 
                                                                            global_dir_dict)
        
              
        curr_wrt_index, num_dirs  = __CommitDirectories(a_file, curr_wrt_index, header_array, 
                                                        global_dir_dict, dir_order)
        ...
        out_index_array = [null_termed_names_bytes,1,num_dirs ,indexing_dict[0][IndexingArray.NUM_STATE_MAPS.value]]
        
        byte_array = struct.pack("4i", *out_index_array)
        a_file.write(byte_array)
        a_file.close();
    if __debug__:
        """
